...
-- Type your code below:
 --  Checking null values in movie column
SELECT 
...
SELECT COUNT(title) AS movie_produced, year
from movie
...
SELECT genre ,
COUNT(genre) AS highest_numbers 
FROM genre
GROUP BY genre
ORDER BY highest_numbers DESC;
...
WITH one_genre 
AS (
SELECT movie_id AS movie_genre,
COUNT(genre) AS genre_count
FROM genre
GROUP BY movie_genre
HAVING genre_count = 1
)
 SELECT COUNT(movie_genre) AS movie_genre
 FROM one_genre;
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
SELECT title, rt.avg_rating, gn.genre
FROM movie as mv
INNER JOIN ratings as rt
ON mv.id=rt.movie_id
...
-- Type your code below:
SELECT title, year, date_published, rt.median_rating
FROM movie AS mv
INNER JOIN ratings AS rt
ON mv.id=rt.movie_id
...
-- Type your code below:
SELECT title, languages, rt.total_votes
FROM movie AS mv
INNER JOIN ratings as rt
ON mv.id=rt.movie_id
...
+---------------+-------------------+---------------------+----------------------+*/
-- Type your code below:
SELECT SUM(CASE WHEN name IS NULL THEN 1 ELSE 0 
          END) AS name_nulls,
...
SELECT title,
  CASE WHEN avg_rating > 8 THEN 'Superhit movie'
            ...
            WHEN avg_rating < 8 THEN 'Flop movies'
...
SELECT production_company, COUNT(mv.id) AS movie_count,
      RANK() OVER(ORDER BY COUNT(mv.id) DESC) AS prod_comp_rank
FROM movie as mv
INNER JOIN ratings AS rt
ON mv.id=rt.movie_id
...
SELECT name_id AS director_id, name AS director_name, dm.movie_id, duration,
       avg_rating AS Avg_rating, total_votes AS total_votes, avg_rating * total_votes AS rating_count, date_published,
